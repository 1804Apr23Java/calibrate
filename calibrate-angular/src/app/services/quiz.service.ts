...
export class QuizService {
  ...
  public getPreviousQuizzesByUserId(userId: number): Observable<any[]> {
    return this.http.get(`http://ec2-184-72-131-208.compute-1.amazonaws.com:8080/CalibrateBackend/${userId}`).pipe(map((response: Response) => {
      return <any[]>response.json();
    }));
  }
//  /generate/{name}/byRandom/withLibraries/{libraryIds}/numQuestions/{length}
